    ...
    public class ProductStoreName
    {
        ...
        public static void add(string name, string notes)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("productstorename_new", conn))
                {
        ...
        {
            using (SqlConnection conn = new SqlConnection(DBUtil.connectionString))
            using (SqlCommand cmd = new SqlCommand("productstorename_isNameExist", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@" + COL_DB_NAME, SqlDbType.VarChar).Value = name;
        ...
        public static DataTable getByFilter(bool includeInactive, string nameFilter)
        {
            ...
            using (SqlConnection conn = new SqlConnection(DBUtil.connectionString))
            using (SqlCommand cmd = new SqlCommand("productstorename_get_byFilter", conn))
        ...
        public static void update(Guid id, string name, string notes)
        {
            try
            {
                ...
                {
                    using (SqlConnection conn = new SqlConnection(DBUtil.connectionString))
                    using (SqlCommand cmd = new SqlCommand("productstorename_update", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@" + COL_DB_ID, SqlDbType.UniqueIdentifier).Value = id;
                        cmd.Parameters.Add("@" + COL_DB_NAME, SqlDbType.VarChar).Value = name;
