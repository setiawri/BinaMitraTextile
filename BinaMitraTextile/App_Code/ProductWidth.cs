    ...
    public class ProductWidth
    {
        ...
        public static void add(string name)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("productwidth_new", conn))
                {
        ...
        {
            using (SqlConnection conn = new SqlConnection(DBUtil.connectionString))
            using (SqlCommand cmd = new SqlCommand("productwidth_isNameExist", conn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@" + COL_DB_NAME, SqlDbType.VarChar).Value = Name;
        ...
        public static DataTable getByFilter(bool includeInactive, string nameFilter)
        {
            ...
            using (SqlConnection conn = new SqlConnection(DBUtil.connectionString))
            using (SqlCommand cmd = new SqlCommand("productwidth_get_byFilter", conn))
        ...
        public static void update(Guid id, string name)
        {
            try
            {
                ...
                {
                    using (SqlConnection conn = new SqlConnection(DBUtil.connectionString))
                    using (SqlCommand cmd = new SqlCommand("productwidth_update", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@" + COL_DB_ID, SqlDbType.UniqueIdentifier).Value = id;
                        cmd.Parameters.Add("@" + COL_DB_NAME, SqlDbType.VarChar).Value = name;
