    ...
    public class Transport
    {
        ...
        public static void add(string name, string address, string phone1, string phone2, string notes)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("transport_new", DBUtil.ActiveSqlConnection))
                {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("transport_isNameExist", DBUtil.ActiveSqlConnection))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@" + COL_DB_NAME, SqlDbType.VarChar).Value = name;
        ...
        public static DataTable getByFilter(bool includeInactive, string nameFilter)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("transport_get_byFilter", DBUtil.ActiveSqlConnection))
        ...
        public static void update(Guid id, string name, string address, string phone1, string phone2, string notes)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("transport_update", DBUtil.ActiveSqlConnection))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@" + COL_DB_ID, SqlDbType.UniqueIdentifier).Value = id;
                        cmd.Parameters.Add("@" + COL_DB_NAME, SqlDbType.VarChar).Value = name;
